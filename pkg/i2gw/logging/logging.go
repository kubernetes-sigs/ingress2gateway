...
// Package logging provides structured logging utilities for ingress2gateway.
// It wraps Go's standard log/slog package and provides:
// - Pretty colored text output for terminal use
// - JSON output for machine parsing
// - Helper functions for common log attributes
package logging
...
const (
	...
	// FormatJSON outputs machine-parseable JSON.
...
// Init initializes the global slog logger with the given configuration. This should be called once
// at program startup.
func Init(cfg Config) {
	...
	case FormatText:
		handler = NewPrettyHandler(cfg.Output, &slog.HandlerOptions{
			AddSource: true,
			Level:     cfg.Level,
		})
	}
...
// error (case-insensitive). Returns slog.LevelInfo if the string is not recognized.
...
// (case-insensitive). Returns FormatText if the string is not recognized.
...
func ObjectRef(obj client.Object) slog.Attr {
	...
	var ref string
	if key.Namespace != "" {
		ref = fmt.Sprintf("%s:%s/%s", kind, key.Namespace, key.Name)
	} else {
		ref = fmt.Sprintf("%s:%s", kind, key.Name)
	}

	return slog.String("object", ref)
}
