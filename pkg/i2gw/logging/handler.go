...
func (h *PrettyHandler) Handle(_ context.Context, r slog.Record) error {
	...
	// Build the box-formatted output.
	var lines []string
	...
	lines = append(lines, topBorder+colorReset)

	// Message line.
	lines = append(lines, fmt.Sprintf("%s│%s  %s", colorGray, colorReset, r.Message))
	...
	for _, attr := range allAttrs {
		...
		// Use distinct colors for provider and object attributes.
		...
		lines = append(lines,
			fmt.Sprintf(
				...
				colorGray, attr.Key, colorReset,
				valueColor, formatAttrValue(attr.Value), colorReset,
			),
		)
	}
	...
	if h.opts.AddSource && r.PC != 0 {
		...
		bottomBorder = fmt.Sprintf("%s└─%s", colorGray, colorReset)
	}
