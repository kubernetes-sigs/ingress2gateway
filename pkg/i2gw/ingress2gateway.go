...
func ToGatewayAPIResources(ctx context.Context, namespace string, inputFile string, providers []string, providerSpecificConf map[string]map[string]string) ([]GatewayResources, error) {
	...
	providerByName, err := constructProviders(&ProviderConf{
		...
		Namespace:        namespace,
		ProviderSpecific: providerSpecificConf,
	}, providers)
	if err != nil {
		return nil, err
	}
