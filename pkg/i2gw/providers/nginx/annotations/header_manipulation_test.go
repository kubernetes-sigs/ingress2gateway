...
func TestHeaderManipulationFeature(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			errs := HeaderManipulationFeature(notifications.NoopNotify, []networkingv1.Ingress{ingress}, nil, &ir)
			if len(errs) > 0 {
				t.Fatalf("Unexpected errors: %v", errs)
			}
...
func TestHeaderManipulationWithSourceIngressMapping(t *testing.T) {
	...
	errs := HeaderManipulationFeature(notifications.NoopNotify, []networkingv1.Ingress{ingress1, ingress2}, nil, &ir)
	if len(errs) > 0 {
