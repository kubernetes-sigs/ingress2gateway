...
func TestSSLServicesAnnotation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			errs := processSSLServicesAnnotation(notifications.NoopNotify, ingress, tt.annotation, &ir)
			if len(errs) > 0 {
...
func TestSSLServicesFeature(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			errs := SSLServicesFeature(notifications.NoopNotify, tt.ingresses, nil, &ir)
			if len(errs) > 0 {
				t.Errorf("Unexpected errors: %v", errs)
				return
			}
