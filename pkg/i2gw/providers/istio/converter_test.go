...
func Test_resourcesToIRConverter_convertGateway(t *testing.T) {
				...
				Spec: gatewayv1.GatewaySpec{
					...
					Listeners: []gatewayv1.Listener{
						...
						},
						{
							...
							Port:     443,
							Protocol: "HTTPS",
							TLS:      &gatewayv1.ListenerTLSConfig{Mode: common.PtrTo(gatewayv1.TLSModeType("Passthrough"))},
						},
						{
						...
						},
						{
							...
							Port:     443,
							Protocol: "HTTPS",
							TLS:      &gatewayv1.ListenerTLSConfig{Mode: common.PtrTo(gatewayv1.TLSModeType("Terminate"))},
						},
						{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := newResourcesToIRConverter(notifications.NoopNotify)
...
func Test_resourcesToIRConverter_convertVsHTTPRoutes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := &resourcesToIRConverter{ctx: context.Background(), notify: notifications.NoopNotify}
...
func Test_resourcesToIRConverter_generateReferenceGrants(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := &resourcesToIRConverter{notify: notifications.NoopNotify}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := &resourcesToIRConverter{
				...
				notify:         notifications.NoopNotify,
			}
...
func Test_convertHostnames(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			actual := convertHostnames(ctx, notifications.NoopNotify, tc.hostnames, field.NewPath(""))
