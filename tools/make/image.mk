...
BUILDX_CONTEXT = i2gw-build-tools-builder
...
image.build.%: image.verify go.build.linux_$(GOARCH).%
	...
	$(eval BUILD_SUFFIX := --pull -t $(IMAGE):$(TAG) -f $(ROOT_DIR)/tools/docker/$(IMAGES)/Dockerfile _output)
...
image.build.multiarch:
	@$(LOG_TARGET)
	docker buildx build _output -f "$(ROOT_DIR)/tools/docker/$(IMAGES)/Dockerfile" -t "${IMAGE}:${TAG}" --platform "${BUILDX_PLATFORMS}"

.PHONY: image.push.multiarch
image.push.multiarch:
	@$(LOG_TARGET)
	docker buildx build _output -f "$(ROOT_DIR)/tools/docker/$(IMAGES)/Dockerfile" -t "${IMAGE}:${TAG}" --platform "${BUILDX_PLATFORMS}" --sbom=false --provenance=false --push
