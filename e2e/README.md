...
Category 5 proves an implementation can route traffic from those resources, the
provider → implementation combination is transitively covered without a dedicated test.
The only exception is emitter-specific CRDs (Category 4), which are inherently tied to a
particular implementation and therefore require explicit pairing.

#### Category 1 — provider smoke tests (`provider_test.go`)
...
#### Category 3 — multi-provider (`multiprovider_test.go`)
...
#### Category 4 — emitter features (TODO)
...
#### Category 5 — implementation smoke tests (`implementation_test.go`)
