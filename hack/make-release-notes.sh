...
readme="https://github.com/LiorLieberman/ingress2gateway/blob/${TAG}/README.md"
download_base="https://github.com/LiorLieberman/ingress2gateway/releases/download"
...
git log "${previous_version}..${TAG}" --format=%an |
  ...
  sed -E 's,^(\s+[0-9]+\s),- ,g'
echo
echo "(ingress2gateway ${TAG} was tagged on $(date -u).)"
echo
echo '<details>'
...
# "${gopath}/bin/release-notes" kubernetes-sigs ingress2gateway --since "${previous_version}"
echo '</details>'
echo
